...
def get_job_ids(keyword, location):
    job_ids = []
...
def get_job_details(job_id, search):
    print(search)
...
def save_jobs(df, type = "json"):

    file_number = 1
    while True:
        file_name = f'jobs{file_number}.{type}'
        try:
            if (type == "csv"):
...
def main():
    ...
    job_ids = get_job_ids(keyword, location)
    ...
    job_details = [get_job_details(job_id, search) for job_id in job_ids]
